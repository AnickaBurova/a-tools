...
#include "long_macro_expansion_double.h"
#include "long_macro_expansion_triplet.h"
...
#define ___VAL_INDEX_2(a,b) a = b,
#define ___INC_RES_2(a,b) res++;
#define ___ENUM_VALUE_2(a,b) __et::a,
#define ___ENUM_STRING_PAIR_2(a,b) {__et::a,to_snake_case(#a,' ')},

#define ENUMS_2(enum_type,...) \
    enum class enum_type { ___EXECUTE_2(___VAL_INDEX_2,__VA_ARGS__ )}; \
    template<>\
    size_t (get_range_size<enum_type>)()\
    {\
        struct counter{ static constexpr size_t count(){ size_t res = 0;___EXECUTE_2(___INC_RES_2,__VA_ARGS__);return res;}};\
        constexpr size_t size = counter::count();\
        return size; \
    } \
    template<>\
    ...
    {\
        using __et = enum_type;\
        return std::vector<enum_type>{\
            ___EXECUTE_2(___ENUM_VALUE_2,__VA_ARGS__)\
        };\
    }\
    std::string to_string(enum_type value) \
    {\
        using __et = enum_type; \
        static std::map<__et,std::string> _vals{ \
            ___EXECUTE_2(___ENUM_STRING_PAIR_2,__VA_ARGS__) \
        }; \
        auto _res = _vals.find(value); \
 \
        return _res != _vals.end() ? _res->second : std::to_string((int)value); \
...
#define ___VAL_INDEX_3(a,b,c) a = b,
#define ___INC_RES_3(a,b,c) res++;
#define ___ENUM_VALUE_3(a,b,c) __et::a,
#define ___ENUM_STRING_PAIR_3(a,b,c) {__et::a,c},

#define ENUMS_3(enum_type,...) \
    enum class enum_type { ___EXECUTE_3(___VAL_INDEX_3,__VA_ARGS__ )}; \
    template<>\
    size_t (get_range_size<enum_type>)()\
    {\
        struct counter{ static constexpr size_t count(){ size_t res = 0;___EXECUTE_3(___INC_RES_3,__VA_ARGS__);return res;}};\
        constexpr size_t size = counter::count();\
        return size; \
    } \
    template<>\
    ...
    {\
        using __et = enum_type;\
        return std::vector<enum_type>{\
            ___EXECUTE_3(___ENUM_VALUE_3,__VA_ARGS__)\
        };\
    }\
    std::string to_string(enum_type value) \
    {\
        using __et = enum_type; \
        static std::map<__et,std::string> _vals{ \
            ___EXECUTE_3(___ENUM_STRING_PAIR_3,__VA_ARGS__) \
        }; \
        auto _res = _vals.find(value); \
 \
        return _res != _vals.end() ? _res->second : std::to_string((int)value); \
